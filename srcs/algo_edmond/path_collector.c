...
/*   Updated: 2019/10/14 05:37:38 by tlandema         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tree_nod			***ft_path_collector(t_data *data, t_tree_nod *start, int i)
{
	t_llist_nod	*link;
	t_tree_nod	***paths;
	t_door		*tmp;
	...
	while (link)
	{
		tmp = ft_door_data_collector(link->linked_room->door_data,
				data->the_flow);
		if (tmp)
		{
			if (!(paths[i] = (t_tree_nod **)ft_memalloc(sizeof(t_tree_nod *) *
					(tmp->size + 2))))
				return (NULL);
			paths[i][0] = data->start;
			if (ft_get_sub_path(paths[i], link->linked_room, data->the_flow,
					tmp->size))
